...
class CartPageAdmin extends StatefulWidget {
  final UnidadeEscolar unidadeEscolar;

  CartPageAdmin(this.unidadeEscolar);
  @override
  _CartPageAdminState createState() => _CartPageAdminState();
}

class _CartPageAdminState extends State<CartPageAdmin> {
  ...
  iniciaBloc() {
    Provider.of<CartBloc>(context, listen: false)
        .fetchUnidade(context, unidadeEscolar.id)
        .then((_) {
      setState(() {
        _isLoading = false;
      });
  ...
  Widget build(BuildContext context) {
    print("widget.user.nome ${user.nome}");
    ...
    return Scaffold(
        ...
        actions: [
          Container(
            ...
            child:    Text(
              'Valor do pedido R\$ ${formatador.format(bloc.total)} ',
  ...
  showExcluir(BuildContext context, Cart c, CartBloc bloc) {
  ...
  _onClickSalvar(BuildContext context) async {
    ...
    var niv = blocEscola.lista.where((e) => e.id==unidadeEscolar.id);
    ...
    ped.escola = unidadeEscolar.id;
    ped.status = Status.pedidoRealizado;
    ped.isativo = true;
    ped.createdAt = hoje;
    ped.modifiedAt = hoje;
    ped.isaf = false;
    ped.total = bloc.total;
    ped.ischeck = false;
    ped.user=user.nome;
