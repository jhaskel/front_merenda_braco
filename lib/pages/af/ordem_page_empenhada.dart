...
class OrdemPageEmpenhada extends StatefulWidget {
  @override
  _OrdemPageEmpenhadaState createState() => _OrdemPageEmpenhadaState();
}

class _OrdemPageEmpenhadaState extends State<OrdemPageEmpenhada> {
  ...
  iniciaBloc() {
    ...
    if(user.isUnidade()){
      Provider.of<PedidoBloc>(context, listen: false).fetchEscola(context,user.escola).then((_) {
        setState(() {
          _isLoadingPedido = false;
        });});
    }


    Provider.of<AfBloc>(context, listen: false)
        .fetchStatus(context,Status.ordemEmpenhada)
        .then((value) {
      setState(() {
        _isLoadingAf = false;
