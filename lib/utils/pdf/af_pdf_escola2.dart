...
class AfPdfEscola2 extends StatefulWidget {
  ...
  List<String> dias;
  Af af;
  AfPdfEscola2(this.list, this.nomeFor,this.dias,this.af);
  @override
  _AfPdfEscola2State createState() => _AfPdfEscola2State();
}

class _AfPdfEscola2State extends State<AfPdfEscola2> {
  _AfPdfEscola2State();
  ...
  Future<void> _generatePDF() async {
    ...
    DateTime crea = DateTime.parse(widget.af.createdAt);
...
//Add the rows to the grid
    ...
    header.cells[0].value= 'ORDEM nº ${widget.af.code}';
    ...
    for(var k in widget.dias){
      ...
      for(var jh in jj){
        ...
        var kk = widget.list.where((element) => element.escola == jh && element.dia==k && element.quantidade > 0);
        print("3100");
        if(kk.length >0){
          PdfGridRow row3 = grid.rows.add();
          row3.cells[0].value = "${kk.last.nomeescola} dia: ${DateFormat("dd/MM").format(crea)}";
...
//Draw the grid
    ...
    headerElement.graphics.drawString(
        'AF_for-${widget.list.first.ordem}', PdfStandardFont(PdfFontFamily.helvetica, 10),
    ...
    js.context['filename'] = 'af_for-${widget.list.first.ordem}.pdf';
    Timer.run(() {
